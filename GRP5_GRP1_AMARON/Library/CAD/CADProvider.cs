    ...
    public class CADProvider
    {
        ...
        public bool CreateProvider(ENProvider provider)
        {
            ...
            try
            {
                ...
                {
                    cmd.CommandText = "INSERT INTO Users (name, password, email, birthdate, urlImage, empresa, address) values ('" + provider.name + "', '" + provider.pass + "', '" + provider.email + "', '" + provider.birth.ToString("yyyy-MM-dd") + "', '" + provider.url + "', '" + provider.company + "', '" + provider.address + "');";
                    cmd.ExecuteNonQuery();
                }
